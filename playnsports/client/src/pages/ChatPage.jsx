...
const ChatPage = () => {
  ...
  return (
    <div className="h-screen bg-[#060606] text-white flex flex-col" style={{ fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div className="flex flex-1 overflow-hidden relative" style={{ height: 'calc(100vh - 64px)' }}>
        ...
        {/* Sidebar */}
        <div className={`flex flex-col border-r border-white/6 bg-black/20 backdrop-blur-sm
          ${activeConv ? 'hidden md:flex' : 'flex'} w-full md:w-80 flex-shrink-0`}>
        ...
        <div className={`flex flex-col flex-1 ${!activeConv ? 'hidden md:flex' : 'flex'}`}>
          {!activeConv ? (
            ...
            <>
              {/* Chat Header */}
              <div className="flex items-center gap-3 px-5 py-4 border-b border-white/6 bg-black/10 backdrop-blur-sm flex-shrink-0">
                <button
                  onClick={() => { setActiveConv(null); navigate('/chat'); }}
                  className="md:hidden text-gray-500 hover:text-white transition-colors mr-1"
                >
                  ←
                </button>
                ...
                {activeConv.type === 'group' && (
                  <div className="flex -space-x-2">
                    ...
                    {activeConv.participants?.length > 4 && (
                      <div className="w-7 h-7 rounded-full border-2 border-[#060606] bg-white/10 flex items-center justify-center text-xs text-gray-400">
                        +{activeConv.participants.length - 4}
                      </div>
                    )}
                  </div>
              ...
              {/* Input */}
              <div className="px-4 py-4 border-t border-white/6 bg-black/10 backdrop-blur-sm flex-shrink-0">
